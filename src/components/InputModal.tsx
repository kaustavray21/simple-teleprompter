export type ModalConfig =
    | { isOpen: false }
    | { isOpen: true; type: 'input'; message: string; inputValue: string }
    | { isOpen: true; type: 'paste'; message: string; inputValue: string }
    | { isOpen: true; type: 'save-prompt'; message: string; inputValue: string }
    | { isOpen: true; type: 'offline-voice-help'; message: string; inputValue: string }
    | { isOpen: true; type: 'error'; message: string; inputValue: string };
...
interface InputModalProps {
    ...
    onPasteSubmit: () => void;
    onSavePromptSubmit?: (name: string, isTemporary: boolean) => void;
}

export function InputModal({ modalConfig, setModalConfig, onGDocSubmit, onPasteSubmit, onSavePromptSubmit }: InputModalProps) {
    if (!modalConfig.isOpen) return null;

    const close = () => setModalConfig({ isOpen: false });
    ...
    return (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4" onClick={(e) => e.stopPropagation()}>
            <div className="bg-zinc-900 border border-zinc-700 rounded-xl p-6 max-w-md w-full shadow-2xl">
                ...
                {modalConfig.type === 'save-prompt' ? (
                    <div className="flex flex-col gap-4">
                        <p className="text-zinc-300 text-sm">How would you like to store this script?</p>
                        <input
                            type="text"
                            autoFocus
                            ...
                            onKeyDown={e => e.key === 'Enter' && modalConfig.inputValue.trim() && onSavePromptSubmit?.(modalConfig.inputValue, false)}
                        />
                        <div className="flex flex-col sm:flex-row justify-end gap-3 mt-4">
                            <button onClick={() => onSavePromptSubmit?.('', true)} className="px-4 py-2 border border-zinc-600 rounded hover:bg-zinc-800 text-zinc-300 transition w-full sm:w-auto text-sm flex items-center justify-center gap-2">
                                Temporary Session <span className="text-xs text-zinc-500">(24h)</span>
                            </button>
                            <button
                                onClick={() => onSavePromptSubmit?.(modalConfig.inputValue, false)}
                                disabled={!modalConfig.inputValue.trim()}
                                className={`px-4 py-2 rounded transition w-full sm:w-auto text-sm font-semibold flex items-center justify-center gap-2 ${modalConfig.inputValue.trim() ? 'bg-blue-600 hover:bg-blue-500 text-white' : 'bg-blue-600/50 text-white/50 cursor-not-allowed'}`}
                            >
                                Save to Library
                            </button>
                        </div>
                    ...
                    <div className="flex flex-col gap-4 text-zinc-300 text-sm leading-relaxed">
                        ...
                        <div className="flex justify-end gap-3 mt-2">
                            <button onClick={close} className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded transition">Got it</button>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        />
                        <div className="flex justify-end gap-3 mt-2">
                            <button onClick={close} className="px-4 py-2 rounded hover:bg-zinc-800 text-zinc-300 transition">Cancel</button>
                    ...
                    <div className="flex justify-end mt-4">
                        <button onClick={close} className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded transition">Close</button>
                    </div>
                )}
            </div>
        </div>
